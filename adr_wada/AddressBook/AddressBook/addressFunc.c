...
/*データを全件表示する関数*/
unsigned int ViewFunction(addressType* addressTbl, itemType const* const itemTbl, unsigned int count) //countは使わず、同じ値を返すだけ
{
	unsigned char fmt[32];
	unsigned char* charPtr;
	unsigned int* intPtr;
	...
	for (unsigned int target = 0; target < count; target++) { // ここで全件ループ
		ViewOneData(addressTbl, itemTbl, target);
		printf("\n");
	}
	return count;
}
			...
			else
			{
				...
				else if (itemTbl[i].itype == TYPE_INT)
				{
					...
					if (strcmp(itemTbl[i].dispName, "ID") == 0) {
						*intPtr = addressTbl[count - 1].id + 1;
					}
					else {
						printf(u8"%sを入力してください：", itemTbl[i].dispName);
						scanf("%d", &intValue);
